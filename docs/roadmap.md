...
| 15.5.3 | Runtime architecture — roster is the source of truth, not an ambient root config. Positional CLI in `__main__.py`: `operator <name> [url]` (run that bot in that meeting), `operator setup` (wizard, see 15.5.5), `operator list` (print roster + one-liners), bare `operator` (usage + roster). `<name>` dispatch loads `roster/<name>/config.yaml` directly; delete the root `config.yaml`. `.env` stays at repo root, shared across bots. `operator <name>` with no URL auto-opens `meet.new`, captures the assigned slug, joins as that bot (absorbs old 15.5.4's `meet.new` deliverable — same surface). Ship `./operator` bash wrapper at repo root as a provisional launcher until product name is locked and curl installer lands (post-launch). Breaking change: every doc/test reference to `python __main__.py <url>` updates to `operator <name> <url>`. *Implemented session 113; live validation deferred to session 114.* | ✅ (code) | ~2.5h |
