...
| 15.5.4 | Face system + profile banner — deterministic pixel-art face generator. Half-block Unicode (`▀ ▄ █`) + ANSI 256-color, 16×20 pixels, seed = hash(bot name). Module alongside `pipeline/`. Generate + hand-curate portraits for engineer / pm / designer, commit as `roster/*/portrait.txt`. First-run auto-write hook: any `operator <name>` run that finds no portrait file generates + saves it (so contributor-added bots get their face minted on first use and the file becomes the gift they commit). On every `operator <name>` run, print a startup banner = face + one-line identity (agent.name + tagline) + truncated loadout (`MCPs: Linear, GitHub · Skills: 2 · Model: claude-sonnet-4-5`). Reads from the active config. Fallback `--plain` flag prints ASCII-only banner for screen readers / hostile terminals. | ⬜ | ~4h |
| 15.5.5 | `operator setup` wizard — creation flow, not activation. Four steps: (1) **choose a template (fighter-select) or start blank** — grid UI rendered with `rich`, numbered prompt, faces visible; (2) **power-ups — awareness-first toggle** — show the template's bundled MCPs + skills (including commented power-up entries) as a checklist; user toggles what ends up in the new bot. Narrow scope: toggles bundle-declared MCPs and `~/.claude/skills/` linkage. Out of scope: arbitrary MCP discovery, paste-a-URL flow (post-launch); (3) **keys** — prompt for any env vars the new bot needs that aren't already in `.env`, `getpass` for secrets, merge preserving existing keys; (4) **finalize** — atomic-write the new `roster/<new-name>/` folder (config.yaml, README.md stub, .env.example), generate its face via 15.5.4's generator, print `operator <new-name> <url>` as the next-step command. Re-run-safe: name-collision check against existing roster folders. ~250 lines of Python + `rich` as a new dep (batteries-included, already battle-tested). Depends on 15.5.3 (for the positional CLI surface `setup` dispatches into) and 15.5.4 (uses the face generator + pre-baked portraits for fighter-select rendering). | ⬜ | ~4h |
...
| Step | Description | Status | Est. |
|------|-------------|--------|------|
| 16.1 | Rewrite `README.md` — what it is, quick start, architecture, "meetings that produce artifacts" positioning. **Must read as 3 steps** (choose a roster member → add your keys/power-ups → join a meeting); roster gallery (`roster/`) is the entry point; link `claude-code` as the canonical starter. Includes: MCP compatibility notes (tested servers + known quirks), BYOMCP failure patterns and mitigation guidance, one annotated example config. Explicit "Claude Code in your Google Meet" framing in the hero. | ⬜ | ~4h |
...
| 16.4 | Launch campaign prep — draft 3 hero-framing posts (see `docs/mvp-bar.md` Launch Strategy section) tailored to their target channels: (a) "Claude Code in your Google Meet" for r/ClaudeAI + Claude-focused creators; (b) "The AI in my standup filed 3 Linear tickets before I finished talking" for r/ExperiencedDevs + eng newsletters; (c) a role-specific build tied to the second chat-native agent from 15.5.2. Prepare the seeded-PR plan for `roster/` (personas + use cases, 3 PRs queued for week 1). Direct-outreach shortlist: 10–20 Claude Code power users in SF. Identify 2–3 AI/PM newsletter operators for earned or paid mentions (Pika's playbook). | ⬜ | ~3h |

**Phase total: ~11h**

---

**MVP total: ~29.5h remaining (71.25h gross across Phases 10–16, of which ~36.75h done in Phases 10–12 + 15.5.0 + 15.5.1 + 15.5.2 + 15.5.2b + 15.5.2c + 15.5.3 LIVE-VALIDATED end-to-end session 115). Execution order (revised session 113): 12 ✅ → 15.5 ▶ (15.5.3 code-complete, live validation pending → next: 15.5.4 → 15.5.5) → 13 → 15.6 → 14 → 15 → 16. Session 113 replanned the wizard: old 15.5.3 (wizard activates a bundle by copying config to root) and old 15.5.4 (`--preset` flag) were replaced by a three-block structure — 15.5.3 runtime architecture (roster as source of truth, positional CLI `operator <name>`, `meet.new` auto-launch absorbed here), 15.5.4 face system + profile banner (deterministic pixel-art portraits, first-run write, startup banner on every meeting run), 15.5.5 wizard as a *creation* flow (builds new `roster/<name>/` folders, four steps: choose template or blank → toggle power-ups → keys → finalize). Curl installer deferred to post-launch (depends on final product name). Budget tight: ~32h vs. ~3 calendar days to April 19. Slim-down candidates if needed: Phase 14 (packaging) and Phase 15 (cross-platform, macOS-primary at launch; Linux VM test only). 15.6 security uncuttable — "Do not cut 15.6.1". User holding launch date.**
