...
| 11.6 | Conversation history persistence — today `LLMClient._history` is an in-memory Python list dropped on process exit (L6-a audit). Persist history to disk so it survives restarts mid-session. Tight scope: write to a local file (JSONL under `~/.operator/history/<meeting_id>.jsonl` or similar), load on startup if the same session is resumed. Not the full `ConversationStore` protocol + swappable backends — just durable persistence of the current shape. Zero-telemetry commitment unchanged (history stays on user's machine). | ⬜ | ~1.5h |
