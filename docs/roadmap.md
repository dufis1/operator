...
*The "choose your fighter → add power-ups → go" layer. Implicit differentiator vs. joinly's framework-shaped positioning. See `docs/mvp-bar.md` for strategy context. **Sequenced before Phase 13 (session 105):** finishes building all new product surface so polish, security, and cross-platform testing run against the final code.*
...
| 15.5.2 | Second chat-native agent — `standup-pm` (committed session 105). Listens during a standup, files Linear tickets from spoken commitments ("I'll do X"), tracks blockers ("blocked on Y"), posts a structured summary at the end. Pairs with `claude-code` to cover dev + PM personas and read + write tool use. Directly serves hero framing #2 ("AI in my standup filed 3 Linear tickets before I finished talking"). Other candidates (debriefer, spec-scribe, incident-commander, customer-call, research) deferred to week-1 seeded PRs. Translator considered but voice-native. **Sequenced before the wizard so the gallery has both fighters when 15.5.3 is written.** | ⬜ | ~1.5h |
...
**MVP total: ~33.5h remaining (61.5h gross across Phases 10–16, of which ~25.25h done in Phases 10 + 11). Execution order (revised session 105): 12 → 15.5 → 13 → 15.6 → 14 → 15 → 16. User holding launch date April 19 with no triage; all four new session-105 items (`standup-pm`, auto-launch, security pass, landing page) stay in.**
...
| Agents gallery expansion | 15.5 | Grow `agents/` beyond the 2 launch agents. **Week-1 seeded PRs (committed session 105):** `debriefer` (joins with prior-meeting context via Granola or local meeting-record search), `spec-scribe` (drafts a PRD from product discussion, opens GitHub PR with the doc), `incident-commander` (opens GitHub issues during incident calls, cross-links, pings oncall), `customer-call` (joins sales/CS calls, files feature requests as Linear tickets in real time), `research` (live web lookup via Tavily MCP, drops citations mid-meeting). **Heavier candidates pending the security pass:** `claude-code-bash` (claude-code agent + bash/shell MCP for live code execution), `scraper` (Playwright MCP for live web scraping), `computer-use` (Anthropic computer-use). Translator and Shopify-expert also queued as voice/vertical examples. Curate community contributions, maintain an `awesome-operator` list for externally-hosted agents. Ongoing post-launch. |
