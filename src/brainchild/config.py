...
TOOL_HEARTBEAT_SECONDS     = 30    # fixed cadence for "still working..." pings during a long tool call. Steady cadence reads as alive; exponential backoff (session 162 and earlier) read as hung after the third stretch.
TOOL_HEARTBEAT_MAX_SECONDS = 30    # retained for back-compat with tests; equals TOOL_HEARTBEAT_SECONDS now that cadence is fixed.
