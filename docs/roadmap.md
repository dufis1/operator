...
| 15.5.5 | `operator setup` wizard — creation + reconfigure flow. See `docs/plan.md` for the locked plan. **Pre-step landed (session 118):** MCP `enabled: true/false` flag on every block in every bot's `config.yaml`; `config.py` filters `enabled: false` at load. Replaces commented-out power-up blocks with real parseable YAML so the wizard reads one data shape. pm is the from-scratch baseline. **Wizard itself (pending):** Five steps — (1) fighter-select with two modes (start-from-scratch prompts name/trigger/tagline; pick-a-preset inherits name and edits in place); (2) power-ups — numbered-list `[x]`/`[ ]` toggle of that bot's MCP blocks (flips `enabled`); (3) skills — user-added paths (folder or single `.md`; single-file case wraps in a stub folder as `SKILL.md`) + bundled skills from the base bot rendered as the same numbered-list toggle (all checked by default, copied into `agents/<name>/skills/`); (4) API keys — prompt for any `${VAR}` referenced by an enabled MCP that isn't already in `.env`, append on confirm, never overwrite without explicit confirmation; (5) atomic write — build bundle in tempdir, `os.rename` into `agents/<name>/`; edit-in-place mode uses `.bak-<timestamp>` swap for recovery. `rich` as a new dep, no `questionary`. `pipeline/setup.py` + `tests/test_setup.py`. Depends on 15.5.3 (positional CLI), 15.5.4 (uses `face.write_if_missing()` for portrait), and the pre-step refactor (clean MCP shape). | ⬜ (pre-step ✅, wizard pending) | ~4h |
