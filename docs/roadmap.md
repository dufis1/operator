...
| 15.5.5 | `operator setup` wizard — creation flow, not activation. Four steps: (1) **choose a template (fighter-select) or start blank** — grid UI rendered with `rich`, numbered prompt, faces visible; (2) **power-ups — awareness-first toggle** — show the template's bundled MCPs + skills (including commented power-up entries) as a checklist; user toggles what ends up in the new bot. Narrow scope: toggles bundle-declared MCPs and `~/.claude/skills/` linkage. Out of scope: arbitrary MCP discovery, paste-a-URL flow (post-launch); (3) **keys** — prompt for any env vars the new bot needs that aren't already in `.env`, `getpass` for secrets, merge preserving existing keys; (4) **finalize** — atomic-write the new `agents/<new-name>/` folder (config.yaml, README.md stub, .env.example), generate its face via 15.5.4's generator, print `operator <new-name> <url>` as the next-step command. Re-run-safe: name-collision check against existing agents folders. ~250 lines of Python + `rich` as a new dep (batteries-included, already battle-tested). Depends on 15.5.3 (for the positional CLI surface `setup` dispatches into) and 15.5.4 (uses the face generator + pre-baked portraits for fighter-select rendering). | ⬜ | ~4h |
