...
| G3 | "Find where auth is handled in [repo]" | Indirect | ❌ | `search_code` returns 0 results on small/new repo (brainchild fixture) even when terms exist in the tree. Confirms pre-existing known fail mode. No error raised — silent empty. | Already covered: steer LLM to `get_file_contents` + directory traversal instead of `search_code` on small repos |
...
| G12 | Multi-repo ambiguity ("my brainchild repo") | Edge | ✅ | Unscoped `search_repositories(query="brainchild in:name")` returns **63,680 repos** — first 5 all Kubernetes brainchilds, none the user's. Scoping with `user:dufis1` narrows to 1 result cleanly. | Always scope repo search with `user:<login>` (from `get_me`) or `org:<org>` when the user refers to "my" or "our" repos |
...
| # | Fail Mode | Trigger | Severity |
|---|-----------|---------|----------|
| 1 | **Large text file poisons context** | `get_file_contents` on any file >~30KB | 🔴 Critical — single call can consume 80%+ of context window; Brainchild has no auto-archive fallback |
...
| 8 | **Unscoped repo search returns 60k+ results** | `search_repositories(query="brainchild")` without `user:` / `org:` scope | 🟡 Medium — LLM may pick wrong repo when user says "my repo" |
