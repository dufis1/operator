...
- **ScreenCaptureKit TCC entries are tied to codesign identity** — if the `audio_capture` binary is recompiled without a stable `--identifier`, macOS generates a hash-based identity. After a TCC reset or macOS update, the old identity's permission entry becomes stale — `startCapture` silently hangs forever (no error, no dialog). Fix: always sign with `codesign --force --sign - --identifier com.operator.audio-capture audio_capture` after compiling. The binary has two layers of defense: (1) `CGPreflightScreenCaptureAccess()` pre-flight check that can trigger the permission dialog, and (2) a 10-second watchdog that exits with code 3 if `startCapture` hangs. **Do NOT auto-re-sign the binary at runtime** — re-signing changes the code identity, which invalidates the Screen Recording TCC grant for the parent app (VS Code, Terminal). This was previously attempted as an auto-retry and caused a cascading failure where the first run's re-sign broke all subsequent runs until the user toggled the permission off/on and restarted the terminal app. Never ship `audio_capture` with only a linker-generated signature.
