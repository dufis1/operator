...
def _find_oauth_mcp_config(mcp_name):
    """Find `mcp_name`'s raw yaml config by scanning bots under ~/.brainchild/agents/.

    Returns the dict for the first bot that declares `mcp_name` with auth:oauth,
    or None if no bot declares it that way. The wizard guarantees per-MCP
    fields stay in sync across bots, so picking the first match is fine.
    """
    import yaml
...
def _run_auth(mcp_name):
    ...
    Zero-hang guarantee at meeting join depends on this command existing:
    MCPClient.connect_all fails fast with kind="oauth_needed" when the
    cache is missing, and the banner tells the user to run this.
    """
    import hashlib
    import re
    import time

    srv = _find_oauth_mcp_config(mcp_name)
    if srv is None:
