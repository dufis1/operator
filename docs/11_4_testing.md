...
2. Edit `config.yaml` → `skills:` block:
...
4. Start: `source venv/bin/activate && python __main__.py <meet-url>`.
...
**Steps:** Leave meeting. Edit `config.yaml`:
...
rm -rf /tmp/op-skills
```
Revert `config.yaml` `skills:` back to `paths: []` before moving on.
